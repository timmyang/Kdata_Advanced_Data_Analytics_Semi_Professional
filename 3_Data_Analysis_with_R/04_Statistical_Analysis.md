  ...
  - **\[4\] 확률변수(random variable)**
      ...
      - 확률변수 X의 k차 중심적률(k-th cental moment)
          ...
          - <img src="https://render.githubusercontent.com/render/math?math=$E (X - \mu ) ^{k} = \int (x - \mu) ^{k} f(x) dx$">:
            연속형 변수인 경우
      - 특히, 2차 중심적률 \<img
        src="<https://render.githubusercontent.com/render/math?math=$E(X>
        - ) ^{2} =
        <sup>{2}\(: 모분산(populatino variance) - 기대값의 선형성을 이용하면 <img src="https://render.githubusercontent.com/render/math?math=\)</sup>{2}
        = E(X - ) ^{2}$
      ...
      - 덧셈정리(배반이 아닐 때)
          - 사건 A와 사건B가 동시에 일어날 수 있들 때(교집합이 성립할 때), **\<img
            src=“<https://render.githubusercontent.com/render/math?math=$일어날>
            확률 P(A 또는 B)는 P(A B) = P(A) + P(B) - P(A B)$\>” 로 표현된다**
...
#### 나. 확률분포
  ...
  - **\[2\] 연속형 확률변수**
      ...
      - **다) 지수분포(Exponential distribution)**
          ...
          - <img src="https://render.githubusercontent.com/render/math?math=$f(x) = 1/2 * e^{-x/2}, 지수분포밀도함수 \lambda = 1/2$">
